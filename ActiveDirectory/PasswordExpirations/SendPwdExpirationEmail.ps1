<#	
	...
	===========================================================================
	 ...
	 Created by:   	Bradley Wyatt - Edited by Andrea Corti
	 Version: 	    1.2.0
	 Notes:
...
$users = Get-Aduser -properties Name, PasswordNeverExpires, PasswordExpired, PasswordLastSet, EmailAddress  -SearchBase $SearchBase  -filter { (Enabled -eq 'True') -and (PasswordNeverExpires -eq 'False')  } # | Where-Object { $_.PasswordExpired -eq $False }
...
foreach ($user in $users)
{
	...
	If (($user.PasswordExpired) -or (($daystoexpire -ge "0") -and ($daystoexpire -lt $expireindays)))
	{
		"$Date - INFO: Sending expiry notice email to $Name" | Out-File ($DirPath + "\" + "Log.txt") -Append
		Write-Host "Sending Password expiry email to $name" -ForegroundColor White
		...
		#Email subject - CHANGE THIS!
        if($user.PasswordExpired){
		    $mailmessage.Subject = "Password expired!"
        }
        else{
		    $mailmessage.Subject = "Password expiring in $daystoexpire days"
        }
		...
		#Email body - CHANGE THIS!
        if($user.PasswordExpired){
		    $mailmessage.Body = "Dear $Name, The password for our domain expired! Please contact the IT department to help you change it"
            
            "Password for $Name EXPIRED!" | Out-File ($DirPath + "\" + "Log.txt") -Append
		    Write-Host "Password for $Name EXPIRED!" -ForegroundColor Magenta
        }
        else{
            "Password for $Name EXPIRES in $daystoexpire days" | Out-File ($DirPath + "\" + "Log.txt") -Append
		    Write-Host "Password for $Name EXPIRES in $daystoexpire days" -ForegroundColor Magenta
